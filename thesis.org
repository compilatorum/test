...
Impure imaginary code is very obviously useful as such code utilises LMs that are trained to perform useful tasks. GPT-3, for example, is a generalist and only requires a tiny amount of prompt design and/or fine-tuning to direct it to perform the task you want.
...
The following prompt function definition function associates a prompt with a LM (OpenAI’s GPT-3 davinci) and defines the parameters for a function in emacs lisp.
...
The following is the generated documentation for the interactive prompt function in emacs.
            ...
            (if
                ...
                (cond
                 ...
                 (t
                  (etv result)))
              result)))))))
The above function creates a NL shell. This enables you to generate shell commands based on NL and it is parameterized to enable you to specify the operating system that the commands generated should run on.

(list2str (pf-bash-one-liner-generator-on-os-from-natural-language "Arch Linux" "Disable firewall" :no-select-result t))
Here is a list of suggestions generated from the above prompt function.
...
- "(pen-selected-text)"
filter: on
A demonstration of two people who understand different world languages using a common LM to understand one another.
...
Database prompt
Find a useful impure imaginary algorithm
With one Pen.el system
...
Pure imaginary programming is a type of programming where the original language models may not even be known.
...
It’s important to avoid mixing training data of varying licenses when training LMs.
...
I am able to generate parts of GPL protected software using LMs and can query the LMs as to how they are used.
...
Imaginary programming is more of an activity and a style of programming and is not really concerned with the amount of uncertainty.
